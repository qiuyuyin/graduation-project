    ...
    int internal_max_size /* = -1*/, int leaf_max_size /* = -1 */)
{
  ...
  int attr_total_length = 0;
  int attr_num = field_metas.size();
  AttrType attr_types[MAX_NUM];
  int32_t length[MAX_NUM];
  int attr_lengths[MAX_NUM];

  for (int i = 0; i < attr_num; i++) {
    attr_total_length += field_metas[i]->len();
    attr_types[i] = field_metas[i]->type();
    attr_lengths[i] = field_metas[i]->len();
  }
  if (rc != RC::SUCCESS) {
  ...
  file_header->attr_length = attr_total_length;
  ...
  key_comparator_.init(file_header->attr_types, file_header->attr_lengths, attr_num);
  key_printer_.init(file_header->attr_types, file_header->attr_lengths, attr_num);
...
RC BplusTreeHandler::open(const char *file_name)
{
  ...
  key_comparator_.init(file_header_.attr_types, file_header_.attr_lengths, file_header_.attr_nums);
  key_printer_.init(file_header_.attr_types, file_header_.attr_lengths, file_header_.attr_nums);
...
// deal with continuous key
char *BplusTreeHandler::make_key(const char *con_key, const RID &rid)
{
  ...
  memcpy(key, con_key, file_header_.attr_length);
...
char *BplusTreeHandler::make_key(const std::vector<const char *> &user_key, const RID &rid)
{
  char *key = (char *)mem_pool_item_->alloc();
  if (key == nullptr) {
    LOG_WARN("Failed to alloc memory for key.");
  ...
  const int sz = user_key.size();
  int offset = 0;
  for (int i = 0; i < sz; i++) {
    auto attr_length = file_header_.attr_lengths[i];
    memcpy(key + offset, user_key[i], attr_length);
    offset += attr_length;
  }
  memcpy(key + file_header_.attr_length, &rid, sizeof(rid));
  return key;
...
RC BplusTreeHandler::insert_entry(const std::vector<const char *> &user_key, const RID *rid)
{
    ...
    const std::vector<const char *> &user_key, const std::vector<int> &key_lens, std::list<RID> &rids)
{
  ...
  RC rc = scanner.open(user_key, key_lens, true /*left_inclusive*/, user_key, key_lens, true /*right_inclusive*/);
  if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::delete_entry(const std::vector<const char *> &user_key, const RID *rid)
{
  auto key = make_key(user_key, *rid);
...
RC BplusTreeScanner::open(const std::vector<const char *> &left_user_key, const std::vector<int> &left_lens,
    bool left_inclusive, const std::vector<const char *> &right_user_key, const std::vector<int> &right_lens,
    bool right_inclusive)
{
  ...
  // 校验输入的键值是否是合法范围
  if (!left_user_key.empty() && !right_user_key.empty()) {
...
RC BplusTreeScanner::fix_user_key(const std::vector<const char *> &user_key, const std::vector<int> &key_lens,
    bool want_greater, std::vector<const char *> &fixed_key, bool *should_inclusive)
{
  if (user_key.empty() || nullptr == should_inclusive) {
    return RC::INVALID_ARGUMENT;
  }

  // 这里很粗暴，变长字段才需要做调整，其它默认都不需要做调整
  // assert(tree_handler_.file_header_.attr_types == CHARS);
  // assert(strlen(user_key) >= static_cast<size_t>(key_len));

  *should_inclusive = false;
  fixed_key.resize(user_key.size());

  const auto field_num = user_key.size();
  for (int i = 0; i < field_num; i++) {
    if (tree_handler_.file_header_.attr_types[i] != CHARS) {
      fixed_key[i] = user_key[i];
      continue;
    }
    const auto char_max_length = tree_handler_.file_header_.attr_lengths[i];
    const auto key_len = key_lens[i];
    char *key_buf = new (std::nothrow) char[char_max_length];
    if (nullptr == key_buf) {
      return RC::NOMEM;
    }

    if (key_len <= char_max_length) {
      memcpy(key_buf, user_key[i], key_len);
      memset(key_buf + key_len, 0, char_max_length - key_len);

      fixed_key[i] = key_buf;
    } else {
      // key_len > char_max_length
      memcpy(key_buf, user_key[i], char_max_length);

      char c = user_key[i][char_max_length];
      if (c == 0) {
        fixed_key[i] = key_buf;
      } else {
        // 扫描 >=/> user_key 的数据
        // 示例：>=/> ABCD1 的数据，char_max_length=4,
        //      等价于扫描 >= ABCE 的数据
        // (类似小数向上取整)
        // 如果是扫描 <=/< user_key的数据
        // 示例：<=/< ABCD1  <==> <= ABCD  (char_max_length=4)
        // (类似小数向下取整)
        *should_inclusive = true;
        if (want_greater) {
          key_buf[char_max_length - 1]++;
        }
