...
RC Db::recover()
{
  ...
  if ((rc = clog_manager_->recover()) == RC::SUCCESS) {
    ...
    for (auto it = mtr_manager->log_redo_list.begin(); it != mtr_manager->log_redo_list.end(); it++) {
      ...
      auto log_type = clog_record->get_log_type();
      if (log_type != CLogType::REDO_INSERT && log_type != CLogType::REDO_DELETE && log_type != CLogType::REDO_UPDATE) {
        delete clog_record;
        continue;
      }
      ...
      switch(clog_record->get_log_type()) {
        ...
        case CLogType::REDO_UPDATE: {
          ...
          Record record;
          record.set_data(record_data);
          record.set_rid(clog_record->log_record_.up.rid_);
